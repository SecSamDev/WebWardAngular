...
export class ScanCheckService {
  ...
  getScanChecks(): Observable<ScanCheck[]> {
    return this.http.get(this.AppSettings.API_ENDPOINT + 'check').map(data => data as ScanCheck[]);
  }
  getScanCheck(id: string): Observable<ScanCheck> {
    return this.http.get(this.AppSettings.API_ENDPOINT + 'check/' + id).map(data => data as ScanCheck);
  }
  postScanCheck(check: ScanCheck) {
    return new Observable(observer=>{
      this.http.post(this.AppSettings.API_ENDPOINT + 'check', check,{responseType : 'json'})
        .subscribe(data=>{
          this.notify();
          observer.next(data);
        },err=>{
  ...
  updateScanCheck(check: ScanCheck) {
    return new Observable(observer=>{
      this.http.put(this.AppSettings.API_ENDPOINT + 'check/' + check.id, check,{responseType : 'json'})
        .subscribe(data=>{
          this.notify();
          observer.next(data);
        },err=>{
  ...
  deleteScanCheck(check: ScanCheck) {
    ...
    return new Observable(observer=>{
      this.http.delete(this.AppSettings.API_ENDPOINT + 'check/' + check.id,{responseType : 'json'})
        .subscribe(data=>{
          this.notify();
          observer.next(data);
        },err=>{
